...
public class AprobarQuimica {
    ...
    public static void main(String[] args) {
        ...
        // Subniveles de acuerdo al diagrama de Moëller
        String[] subniveles = {
        ...
        while (true) {
            ...
            int numeroAtomico = Integer.parseInt(scanner.nextLine());

            // Si se encuentra "Exit", termina el programa
            if (nombreElemento.equals("Exit")) {
                break;
            }

            // Caso especial cuando Z = 0
            if (numeroAtomico == 0) {
                System.out.println("1s0");
                continue;
            }

            // Variables para la configuración electrónica
            ...
            for (int i = 0; i < subniveles.length; i++) {
                ...
                // Agregar el subnivel y la cantidad de electrones
                configuracion.append(subniveles[i]).append(electronesEnEsteSubnivel).append(" ");
            ...
            // Imprimir la configuración sin el último espacio
...
La figura 1.b muestra en qué orden se van completando los subniveles. Como se ve, consiste en recorrer los subniveles de forma diagonal de arriba a abajo y de derecha a izquierda:
...
n=4	2	6	
n=5	1		
La forma de indicar la configuración electrónica es mostrar uno tras otro todos los subniveles que tienen electrones utilizando el orden en el que se han ido rellenando. Además, para cada subnivel se indica el número de electrones que han caído en él. Para nuestro ejemplo será:

1s2 2s2 2p6 3s2 3p6 4s2 3d10 4p6 5s1
El problema consiste en obtener la configuración electrónica de los elementos que nos vaya diciendo Jonathan.

Entrada
La entrada consistirá en una secuencia de casos de prueba, donde cada caso de prueba está formado por dos líneas: el nombre del elemento químico y su número atómico Z. El nombre del elemento químico estará compuesto por una única palabra y no tendrá nunca más de 50 letras; por su parte, el número atómico estará entre 0 y 118.

El programa terminará de recibir valores cuando el nombre del elemento sea "Exit".

Salida
Para cada caso de prueba, el programa indicará la configuración electrónica del elemento introducido. La configuración electrónica será la lista de los subniveles en el orden en el que se van rellenando seguido del número de electrones que hay en ese subnivel. Cada subnivel se separará del anterior por un espacio en blanco.
