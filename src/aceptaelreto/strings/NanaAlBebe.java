...
public class NanaAlBebe {
    ...
    public static void main(String[] args) {
        ...
        while (sc.hasNextLine()) {
            ...
            // Convertir la palabra a minúsculas para que la comparación sea insensible a mayúsculas/minúsculas
    ...
    public static int encontrarSubcadenaCorta(String palabra) {
        ...
        for (int i = 1; i <= longitudPalabra; i++) {
            ...
            // Repetir la subcadena suficientes veces para cubrir toda la longitud de la palabra
            for (int j = 0; j < (longitudPalabra / i); j++) {
                palabraRepetida += subcadena;  // Concatenamos las subcadenas manualmente
            }

            // Si la longitud total de la repetición cubre la palabra original, la subcadena es la correcta
            if (palabraRepetida.equals(palabra)) {
                return i; // Devuelve la longitud de la subcadena que se repite
            }
        }

        // Si no encontramos una subcadena (lo cual no debería ocurrir si la palabra es cíclica)
