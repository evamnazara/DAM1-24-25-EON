...
public class IncidenteDhahran {
    ...
    public static String procesarNumero(String numero) {
        ...
        parteEntera = parteEntera.replaceFirst("^0+(?!$)", ""); // Remover ceros a la izquierda, pero aseguramos que no se quede vacío
        
        // 3. Eliminamos los ceros a la derecha de la parte fraccionaria, si existe parte fraccionaria
        if (!parteFraccionaria.isEmpty()) {
            parteFraccionaria = parteFraccionaria.replaceAll("0+$", ""); // Eliminar ceros al final
        }
        
        // 4. Si la parte fraccionaria está vacía después de eliminar ceros, no la añadimos
        if (parteFraccionaria.isEmpty()) {
            return parteEntera; // Solo devolvemos la parte entera
        }
        
        // 5. Si queda parte fraccionaria, devolvemos la parte entera + "." + parte fraccionaria
