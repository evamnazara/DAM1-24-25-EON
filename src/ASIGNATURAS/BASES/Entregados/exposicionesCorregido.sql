...
create table exposicion (
    ...
    tematica varchar(50) not null
		CONSTRAINT DF_EXPOSICIONTEMATICA DEFAULT 'fiestas',
...
create table localidad (
	...
	numHabitantes int
		CONSTRAINT d_habitantes DEFAULT 5000,
	provincia smallint,
...
create table EXPOSICION_SALA (
	...
	fechaInicio date not null
		CONSTRAINT DF_EXPOSICIONSALA_FECHAINICIO DEFAULT GETDATE(),
	fechaFin date null,
...
CREATE TABLE FOTOGRAFO(
	...
	codInfluencer smallint null, --tiene que ser el mismo tipo q la clave  
	...
	CONSTRAINT CK_EDAD_FOTOGRAFO CHECK (  
		DATEDIFF(YEAR, fechaNacimiento, GETDATE()) >= 18 AND 
               (MONTH(GETDATE()) > MONTH(fechaNacimiento) OR
			   (MONTH(GETDATE()) = MONTH(fechaNacimiento) AND 
				DAY(GETDATE()) >= DAY(fechaNacimiento)))
			),
...
alter table CENTRO
	ADD codigoPostal char(5),
	CONSTRAINT DF_CPCENTRO DEFAULT '36965' FOR codigoPostal WITH VALUES,
	CONSTRAINT CK_CPCENTRO CHECK (codigopostal LIKE ('[0-9][0-9][0-9][0-9][0-9]'));
...
alter table EXPOSICION_SALA
	ADD 
		entradasVendidas int
			CONSTRAINT DF_PRECIOEXPSALA DEFAULT 5,		
		precio int,

		CONSTRAINT CK_PRECIOENTRADAS CHECK (Precio <= 20),
...
CREATE TABLE LABORATORIO (
	codLaboratorio smallint not null,
	codFotografo smallint not null,
	premio int,
...
CREATE TABLE FOTOGRAFO_ESTUDIO_LABORATORIO (
	codLaboratorio smallint not null,
	codFotografo smallint not null,
	codEstudio smallint not null,
	
	constraint pk_FOTOGRAFO_ESTUDIO_LABORATORIO PRIMARY KEY (codLaboratorio, codFotografo,codEstudio)
);


GO ALTER DATABASE DBFOTOGRAFIA 
	ADD FILE (
	NAME 
...
/*
	BORRAR Y READONLY
	ALTER COLUMN PAGINAWEB (100)
	ANTES DE BORRAR CAMPOS BORRAR RESTRICCIONES
*/