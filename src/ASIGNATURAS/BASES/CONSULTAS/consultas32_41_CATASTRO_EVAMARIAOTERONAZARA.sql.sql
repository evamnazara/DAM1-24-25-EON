...
SELECT NUMPARQUES, VIVIENDA.CALLE, VIVIENDA.NUMERO,VIVIENDA.METROSSOLAR 
	FROM ZONAURBANA,VIVIENDA
...
SELECT ZU.NOMBREZONA, DESCRIPCION 
	FROM ZONAURBANA ZU 
...
SELECT PP.NOMBRE, PP.APELLIDO1 + ' ' + ISNULL(PP.APELLIDO2, ' ') AS APELLIDOS
	FROM PROPIETARIO PP 
	INNER JOIN HUECO H ON PP.DNI = H.DNIPROPIETARIO
...
SELECT DNI, NOMBRE, APELLIDO1 + ' ' + ISNULL(APELLIDO2, ' ') AS APELLIDOS
	FROM PROPIETARIO PP 
	INNER JOIN CASAPARTICULAR CP ON PP.DNI = CP.DNIPROPIETARIO
...
SELECT P.*, PP.NOMBRE + ' ' + PP.APELLIDO1 + ' ' + ISNULL(PP.APELLIDO2, ' ') AS [NOMBRE COMPLETO]
	...
	WHERE (ZU.NUMPARQUES = 2 AND (P.NUMHABITACIONES BETWEEN 2 AND 4)) 
		OR (ZU.NOMBREZONA = 'Centro' AND BP.ASCENSOR = 'S' AND (P.METROSUTILES BETWEEN 70 AND 110))
GO
