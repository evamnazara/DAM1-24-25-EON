...
SELECT CODIGO, CALLE, NUMERO, PLANTA, PUERTA, ID_HUECO, TIPO, METROS, OBSERVACIONES, DNI_PROPIETARIO = ISNULL(DNIPROPIETARIO,'Desconocido') FROM HUECO 
    WHERE METROS >= 14
GO
...
SELECT nombreCompleto = NOMBRE +' '+ APELLIDO1 +' '+ ISNULL(APELLIDO2,'') FROM PROPIETARIO 
    WHERE NOMBRE LIKE'[^abcde]%' AND LEN(APELLIDO1)>4 
...
SELECT * FROM VIVIENDA 
    WHERE CALLE IN('Damasco', 'General Mola') AND METROSSOLAR 
    LIKE '[2]%'
GO
...
SELECT metros = FLOOR(METROS) FROM HUECO 
